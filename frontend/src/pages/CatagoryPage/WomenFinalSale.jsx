...
const WomenFinalSale = () => {
  ...
  useEffect(() => {
    let filteredProducts = products.filter(
      (item) => item.category?.toLowerCase() === 'women' && item.isLastSize
    );
  ...
  return (
    <div className='pt-10 border-t'>
      <div className='w-full mb-4'>
        <img src={assets.womenimg} className='w-full h-60 object-cover rounded-xl' alt="Women Final Sale Banner" />
      </div>

      <div className="w-full overflow-x-auto scrollbar-hide py-4 mb-6">
        <div className="flex gap-4 min-w-max px-2 sm:px-4">
          {[
            { label: 'TẤT CẢ ÁO', img: assets.aothunnu, link: '/women/final-sale/ÁO' },
            { label: 'TẤT CẢ QUẦN', img: assets.quannu, link: '/women/final-sale/QUẦN' },
            { label: 'TẤT CẢ VÁY / ĐẦM', img: assets.vaynu, link: '/women/final-sale/VÁY-ĐẦM' },
          ].map((item, index) => (
