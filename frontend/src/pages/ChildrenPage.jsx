...
const ChildrenPage = () => {
  ...
  const applyFilter = () => {
    let productsCopy = products.filter(
      (item) => item.category && item.category.toLowerCase() === 'children'
    );
...
<div className="w-full overflow-x-auto scrollbar-hide py-4 mb-6">
  ...
  {[
  { label: 'SALE', img: assets.sale, link: '/children/final-sale/ao' },
  { label: 'ÁO', img: assets.aothunkid, link: '/children/category/ao' },
  { label: 'QUẦN', img: assets.quankid, link: '/children/pants/quan' },
].map((item, index) => {
  const content = (
    ...
    <div key={index}>
      {content}
        ...
        <div className='flex justify-between items-center mb-4'>
          <Title text1={'CHILDREN'} text2={'COLLECTION'} />
          <select
