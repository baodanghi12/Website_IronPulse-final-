  ...
  return (
    <form onSubmit={onSubmitHandler} className="flex flex-col items-center w-[90%] sm:max-w-[400px] m-auto mt-14 gap-6 text-gray-800 bg-white p-8 rounded-xl shadow-lg">
      ...
      <div className="w-full flex justify-between text-sm mt-[-8px]">
        ...
        {currentState === 'Login' ? (
          <p onClick={() => setCurrentState('Sign Up')} className="cursor-pointer text-gray-600 hover:text-purple-600">
            Create account
          </p>
        ) : (
          <p onClick={() => setCurrentState('Login')} className="cursor-pointer text-gray-600 hover:text-purple-600">
            Login Here
          </p>
        )}
      </div>
